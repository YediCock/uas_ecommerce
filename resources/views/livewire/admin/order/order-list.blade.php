            ...
            <div class="widget-content widget-content-area">
                <div class="filtered-list-search mb-2 w-50">
                    <form class="form-inline my-2 my-lg-0 justify-content-center">
                        <div class="w-100">
                            <input wire:model.live="search" type="text" class="py-2 w-100 form-control product-search br-30" id="input-search" placeholder="Cari transaksi order...">
                        </div>
                    </form>
                </div>
            ...
            <div class="modal-content">
                @if ($selectedOrder)
                    <div class="modal-header">
                        <h5 class="modal-title fs-6" id="exampleModalLabel">Detail order {{ $selectedOrder->name }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body py-2">
                        <p class="fw-bold text-decoration-underline">Detail Pesanan</p>
                        <div class="">
                            <p>Status pesanan : {{ $selectedOrder->status }}</p>
                            <p>Tanggal pembelian : {{ $selectedOrder->created_at->format('d F Y') }}</p>
                            <p>Kurir : {{ $selectedOrder->shipping_courier }} - {{ $selectedOrder->shipping_service_name }}</p>
                            <p>Nomer Resi : {{ $selectedOrder->track_number ?? '-' }}</p>
                        </div>
                        <p class="mt-3 fw-bold text-decoration-underline">Alamat Pengiriman</p>
                        <div class="">
                            <p>Nama Penerima : {{ $selectedOrder->customer_name }}</p>
                            <p>Nomer Handphone : {{ $selectedOrder->phone }}</p>
                            <p>Alamat : {{ $selectedOrder->address }}</p>
                        </div>
                        <p class="mt-3 fw-bold text-decoration-underline">Detail Produk</p>
                        <div class="">
                            <p>Nama Penerima : {{ $selectedOrder->customer_name }}</p>
                            <p>Nomer Handphone : {{ $selectedOrder->phone }}</p>
                            <p>Alamat : {{ $selectedOrder->address }}</p>
